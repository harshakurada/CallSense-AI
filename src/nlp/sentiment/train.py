"""Trains and saves both the baseline and fine-tuned sentiment classifier."""
...
from src.nlp.sentiment.dataset import load_sentiment_dataset
from src.utils.logging import get_logger

logger = get_logger(__name__)

MODEL_DIR = Path("models/nlp/sentiment")


def train(num_epochs: int = 3) -> dict:
    data = load_sentiment_dataset()
    ...
    logger.info(
        "Sentiment: %d train / %d val / %d test, classes=%s, full distribution=%s",
    ...
    logger.info("Sentiment baseline: accuracy=%.4f macro_f1=%.4f", baseline_metrics.accuracy, baseline_metrics.f1_macro)
    ...
    logger.info(
        "Sentiment transformer: accuracy=%.4f macro_f1=%.4f",
